...
ODBCDoc.h，ODBCDoc.cpp - 文档
这些文件包含 CODBCDoc 类。编辑这些文件可以添加特殊文档数据并可实现文件保存和加载(通过 CODBCDoc::Serialize)。

ODBCView.h，ODBCView.cpp - 文档的视图
这些文件包含 CODBCView 类。
CODBCView 对象用于查看 CODBCDoc 对象。
...
ODBCSet.h, ODBCSet.cpp
这些文件包含 CODBCSet 类。此类用于访问您在向导中选择的数据源。
已添加序列化支持。
